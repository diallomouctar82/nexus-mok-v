...
import { StrategicPlan, ViewType, AppAction, SovereignSnapshot, Artifact, GroundingSource } from '../types';
...
// Fixed: Defined missing StrategicHubProps interface
...
export const StrategicHub: React.FC<StrategicHubProps> = ({ onNavigate, onExecute, externalGoal, onGoalConsumed }) => {
  ...
  const forgeArtifact = async (type: string) => {
    ...
    setIsGenerating(true);
    try {
      const art = await gemini.generateProjectArtifact(plan, type, "Conception initiale terminée.");
  ...
  const researchDonors = async () => {
    ...
    try {
      ...
      // Créer un artefact automatiquement
      const art: Artifact = {
        ...
        <div className="absolute inset-0 artifact-viewer p-20 flex flex-col animate-in fade-in duration-500">
           <header className="flex justify-between items-center mb-10 pb-8 border-b border-white/10">
              <div className="flex items-center gap-6">
                 <div className="w-16 h-16 bg-blue-600 rounded-3xl flex items-center justify-center text-3xl"><i className="fas fa-file-contract"></i></div>
                 <div>
                    <h2 className="text-3xl font-black italic uppercase">{selectedArtifact.title}</h2>
                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{selectedArtifact.type} • Scellé par le Nexus</span>
                 </div>
              </div>
              <button onClick={() => setSelectedArtifact(null)} className="w-14 h-14 bg-white/5 rounded-2xl flex items-center justify-center hover:bg-red-600 transition-all text-xl"><i className="fas fa-times"></i></button>
           </header>
      ...
      <header className="flex items-center justify-between z-10 mb-12">
        <div className="flex items-center gap-10">
          ...
          <div>
            ...
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.5em] mt-3">Gestion End-to-End du Cycle Souverain</p>
          </div>
        </div>
        {plan && (
      ...
      {!plan ? (
        <div className="flex-1 flex flex-col items-center justify-center max-w-4xl mx-auto space-y-16 animate-in zoom-in-95 duration-700">
           <div className="text-center space-y-8">
              ...
              <p className="text-slate-400 text-2xl font-medium max-w-2xl mx-auto italic">De la conception à la levée de fonds, le Nexus orchestre votre succès.</p>
           </div>
           ...
           <aside className="space-y-8 flex flex-col h-full">
              <div className="mission-control-panel p-10 flex-1 flex flex-col gap-8 overflow-hidden">
                 <h4 className="text-[11px] font-black uppercase tracking-widest text-blue-400 border-b border-white/5 pb-6 flex items-center justify-between">
                    ...
                    <span className="text-slate-600">{artifacts.length} scellés</span>
                 </h4>
                 <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2">
                    {artifacts.map(art => (
                      <div key={art.id} onClick={() => setSelectedArtifact(art)} className="artifact-item p-6 group">
                         <div className="flex justify-between items-center">
                            <div className="flex flex-col">
                               <span className="text-[11px] font-black text-white uppercase truncate max-w-[200px]">{art.title}</span>
                    ...
                    {artifacts.length === 0 && <div className="text-center py-20 opacity-10 italic text-[10px] uppercase">Aucun document généré</div>}
                 </div>
              </div>

              <div className="p-10 bg-black/60 border border-blue-500/20 rounded-[3.5rem] space-y-8">
                 <h4 className="text-[10px] font-black uppercase tracking-widest text-slate-500">Actions Souveraines</h4>
                 <div className="grid grid-cols-1 gap-4">
                    <button 
                      onClick={researchDonors} 
                      disabled={isResearching}
                      className={`w-full py-5 bg-emerald-600 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-xl flex items-center justify-center gap-4 ${isResearching ? 'searching-animation' : ''}`}
                    >
                       <i className="fas fa-magnifying-glass-dollar"></i>
                       {isResearching ? 'Recherche en cours...' : 'Rechercher Bailleurs'}
                    </button>
                    <button onClick={() => forgeArtifact('Rapport Final de Mission')} className="w-full py-5 bg-white/5 border border-white/10 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-blue-600 transition-all">
